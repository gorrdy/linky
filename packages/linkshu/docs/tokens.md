...
`forget` closes a transfer the caller has nothing left to do about: a send whose token verifiably reached its recipient (a published message), or a `receive` in `pending`/`failed` that will never be retried. It is not a refund — the handed-out proofs stay `handedOut` and are still reported `spent` once the recipient claims them; until the mint reports that, `returnToWallet` can still take the send back. Everything else (`pending` → `done` on a receive, `spent` marking, closing claimed sends) is done by the operation verticals. Platforms never write states themselves.
...
| `send` in `done` with a proof still `handedOut` or `externalized` (closed by `forget`, not claimed) | re-received exactly like an open send                                                                                                                                                                         |
