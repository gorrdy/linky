...
Redaction runs inside lazy `emit` builders, so a disabled inspector never traverses drafts or results. The stripping is `redactInspectorSecrets` (`src/internal/redactInspectorSecrets.ts`), applied inside `inspectOperation`, `inspectPlainOperation`, the outbox enqueue row, and the inbox routing row; it walks the value, drops `key`/`nonce` from any object that also has `encryptionAlgorithm`, and replaces a `token` field holding an encoded cashu token (`cashuA`/`cashuB`) — a bearer instrument — with a placeholder. `src/inspector/inspectorEmission.test.ts` sends, enqueues, receives, and fetches both an image and a cashu token, asserting no emitted row contains the key bytes or the token text. Keep it that way when you add an emission point: pass drafts, receipts, ids, and errors through those helpers (or through `redactInspectorSecrets` when emitting by hand), never the identity service or a config object.
