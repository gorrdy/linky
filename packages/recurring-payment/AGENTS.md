...
- Ids are plain strings. `readRecurringPaymentOrder` and the planner are generic so an app's branded ids survive the round trip; do not add a dependency on `@linky/linksync` for them.
